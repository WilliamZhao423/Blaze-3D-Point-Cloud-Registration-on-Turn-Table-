...
MapBuilder::MapBuilder() :
    ...
    submap_size_(300), voxel_grid_leaf_size_(1.5), map_update_distance_(1), enable_optimize_(true),
...
void MapBuilder::downSample(const pcl::PointCloud<pcl::PointXYZ>::Ptr& input_cloud,
                            pcl::PointCloud<pcl::PointXYZ>::Ptr& sampled_cloud)   // TODO @Yimin Zhao                         
{
    pcl::VoxelGrid<pcl::PointXYZ> voxel_grid_filter;   // TODO @Yimin Zhao
...
void MapBuilder::detectLoopClosure(const KeyFrame::Ptr& key_frame)
{
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr sampled_cloud(new pcl::PointCloud<pcl::PointXYZ>()); // TODO @Yimin Zhao
    downSample(key_frame->getCloud(), sampled_cloud);

    // 从多个回环链中遍历每一个回环链
    for(const std::vector<KeyFrame::Ptr>& chain : cloud_chains) {
    
        pcl::PointCloud<pcl::PointXYZ>::Ptr target_cloud(new pcl::PointCloud<pcl::PointXYZ>());  // TODO @Yimin Zhao

        // 构建局部地图，向target_cloud中添加每个关键帧的点云
        for(const KeyFrame::Ptr& frame : chain) {
            pcl::PointCloud<pcl::PointXYZ> transformed_cloud; // TODO @Yimin Zhao
        ...
        pcl::PointCloud<pcl::PointXYZ> output_cloud;   // TODO @Yimin Zhao
        pcl::NormalDistributionsTransform<pcl::PointXYZ, pcl::PointXYZ> ndt;   // TODO @Yimin Zhao
...
//void MapBuilder::addPointCloud(const pcl::PointCloud<pcl::PointXYZ>::Ptr& point_cloud, const Eigen::Matrix4f& guess_transform) // TODO
void MapBuilder::addPointCloud(const pcl::PointCloud<pcl::PointXYZ>::Ptr& point_cloud, double angley) // TODO
{
    ...
    //std::cout << "guess_transfom " << guess_transform << std::endl;
    ...
    if(first_point_cloud_) {
        ...
        map_ += *point_cloud;
        map__+= *point_cloud; // TODO @2022.11.17
        submap_.push_back(point_cloud);  
        //pcl::io::savePCDFile<pcl::PointXYZ> ("/home/yimim/Public/pcl_test/NDT/build/2.pcd", *point_cloud); 
        //*target_cloud = *point_cloud;   
        ndt_.setInputTarget(point_cloud); // TODO @2022.11.04
    ...
    //std::cout << "sampled_cloud->size()" << sampled_cloud->size() << std::endl;
    // 通过ndt算法，得到变换矩阵，作为优化的起始位姿
    pcl::PointCloud<pcl::PointXYZ>::Ptr output_cloud (new pcl::PointCloud<pcl::PointXYZ>); // TODO @Yimin Zhao
    //ndt_.setInputSource(sampled_cloud);
    ndt_.setInputSource(point_cloud);
    
    // Calculating required rigid transform to align the input cloud to the target cloud.
    ndt_.align(*output_cloud, guess_transform); // TODO @Yimin Zhao  
    
    // Transforming point_cloud using found transform.
    //pcl::transformPointCloud (*point_cloud, *target_cloud, ndt_.getFinalTransformation());
    
    // Transforming point_cloud using guess transform
    pcl::transformPointCloud (*point_cloud, *target_cloud, guess_transform); 
    
    //Transforming sampled_cloud using found transform.
    //pcl::transformPointCloud (*sampled_cloud, *output_cloud, ndt_.getFinalTransformation());
    //pcl::transformPointCloud (*sampled_cloud, *output_cloud, guess_transform);
   
    //if(submap_.size() < submap_size_) {
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr map_cloud(new pcl::PointCloud<pcl::PointXYZ>); // TODO @Yimin Zhao 
    map_cloud->clear();
    ...
    ndt_.setInputTarget(target_cloud); // TODO add by @Yimin Zhao  
    
    pcl::PointCloud<pcl::PointXYZ>::Ptr map_sampled_cloud(new pcl::PointCloud<pcl::PointXYZ>); // TODO @Yimin Zhao
    if(ang > 355){
        // insert points on two ends of the scan point cloud of the object
        // downsample map_cloud
        downSample(map_cloud, map_sampled_cloud);
        pcl::io::savePCDFileASCII ("/home/yimim/Public/pcl_test/NDT/build/map_sampled_cloud.pcd", *map_sampled_cloud);
    ...
    /*if(delta > map_update_distance_) {
        ...
        // 添加局部地图，如果局部地图序列长度大于submap_size_，将最早的点云删除
            ...
            pcl::PointCloud<pcl::PointXYZ>::Ptr transformed_cloud(new pcl::PointCloud<pcl::PointXYZ>()); // TODO @Yimin Zhao
